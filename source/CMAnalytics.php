...
function CMAnalyticsV2($webhookUrl, $username, $avatarUrl, $eventType, $eventIcon, $colorHex, $additionalFields)
{
    ...
    $eventIcon = $eventIcon ?: $defaultUrl;
    $color = $colorHex ? hexdec(ltrim($colorHex, "#")) : 23980;
    ...
    foreach ($botUserAgents as $botSignature) {
        if (preg_match("/$botSignature/i", $userAgent)) {
            exit(); // Exit if a bot is detected
        }
    }

    $ipAddress = $_SERVER["REMOTE_ADDR"];
    $curl = curl_init("http://ip-api.com/json/$ipAddress");
    curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
    $ipInfo = json_decode(curl_exec($curl));
    curl_close($curl);
    if (!$ipInfo) return;
    $timeZone = $ipInfo->timezone;
    if($timeZone) date_default_timezone_set($timeZone);
    $fields = [
        ["name" => "IP Address", "value" => $ipInfo->query, "inline" => true],
        ["name" => "Country", "value" => $ipInfo->country, "inline" => true],
        ["name" => "City", "value" => $ipInfo->city, "inline" => true],
        ["name" => "Service Provider (ISP)", "value" => $ipInfo->isp, "inline" => true],
        ["name" => "ISP Coordinates", "value" => "$ipInfo->lat, $ipInfo->lon", "inline" => true],
        ["name" => "Local Date & Time", "value" => date("m/d/Y • g:i A"), "inline" => true],
        ["name" => "Browser Info", "value" => $_SERVER["HTTP_USER_AGENT"], "inline" => false]
    ];
    ...
    $jsonPayload = json_encode([
        ...
        "embeds" => [
            [
                "description" => "Your CMAnalytics webhook has been triggered. Please check the embed below to view the request details.",
                "color" => $color,
                "author" => [
                    "name" => $eventType,
                    "icon_url" => $eventIcon,
